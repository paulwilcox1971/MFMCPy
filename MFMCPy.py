...
SPEC_TYPE_PREFIX = {
    SEQUENCE_TYPE: '{sequence}' + H5_PATH_SEPARATOR, 
    PROBE_TYPE: '{probe}' + H5_PATH_SEPARATOR, 
    LAW_TYPE: '{law}' + H5_PATH_SEPARATOR}
...
def fn_check_sequence(MFMC, SPEC, sequence_name):
    ...
    if 'TYPE' not in list(sequence_name.attrs) or sequence_name.attrs["TYPE"] != SEQUENCE_TYPE:
    ...
    sequence_specification = fn_get_relevant_part_of_spec(SPEC, SEQUENCE_TYPE)
    ...
    probe_spec = fn_get_relevant_part_of_spec(SPEC, PROBE_TYPE)
    ...
    law_spec = fn_get_relevant_part_of_spec(SPEC, LAW_TYPE)
...
def fn_check_mfmc_group_against_specification(MFMC, SPEC, mfmc_group, spec, check_log = [], size_table = {}, err_list = []):
    ...
    dsets_in_obj = fn_get_dataset_keys(mfmc_group)
    attrs_in_obj = list(mfmc_group.attrs)
    for name in list(spec.index):
        base_string = mfmc_group.name + H5_PATH_SEPARATOR + name + ': '
        ...
        if item_dtype == np.dtype('O'):
            ...
            unique_items = fn_unique_h5_ref_list(MFMC, item)
