...
def fn_probe_test(create_fn, test_fn, input_params, s, obj):
    ...
    try:
        ...
        #m.read.fn_pretty_print_dictionary(input_params)
    except:
    ...
    try:
        ...
        #m.read.fn_pretty_print_dictionary(probe_details)
    except:
    ...
    for p in input_params.keys():
        ...
        for (t1, t2) in zip(tmp1, tmp2):
            if p == m.strs.eng_keys.PITCH and probe['ELEMENT_POSITION'].shape[0] == 1:
...
class cl_test_probe_testers(unittest.TestCase):
    def test_linear(self):
        ...
        for i in range(1, no_trials):
            ...
            input_params[m.strs.eng_keys.ELEMENT_WIDTH] =      (rnd.random() * 0.8 + 0.1) * input_params['Pitch (m)'];
            input_params[m.strs.eng_keys.ELEMENT_LENGTH] =     rnd.random() * 20e-3 + 1e-3;
            input_params[m.strs.eng_keys.NUMBER_OF_ELEMENTS] =   rnd.integers(1, 129)
            ...
            msg_str = ' [Seed = %i]' % i
            #Test it
            if not fn_probe_test(m.write.fn_linear_array, m.read.fn_test_for_1D_linear_probe, input_params, msg_str, self):
                print(msg_str)
