...
#Following needs updating to use dictionary
def fn_matrix_array(input_params):
    p = fn_expand_if_nesc(input_params['Pitch (m)'])
    s = fn_expand_if_nesc(el_size)
    n = fn_expand_if_nesc(no_elements)
    nt = n[0] * n[1]
    second_direction = np.cross(normal_direction, first_direction)
    (t1, t2) = np.meshgrid((np.array(range(n[0])) - (n[0] - 1) / 2) * p[0],  (np.array(range(n[1])) - (n[1] - 1) / 2) * p[1])
    p = np.array(centre_pos).reshape(1, 3) + \
        t1.reshape(nt, 1) * np.array(first_direction).reshape(1, 3) + \
        t2.reshape(nt, 1) * np.array(second_direction).reshape(1, 3)
    e1 = np.tile(first_direction, (nt, 1)) * s[0] / 2
    e2 = np.tile(second_direction, (nt, 1))* s[1] / 2
    return {'ELEMENT_POSITION': p, 'ELEMENT_MAJOR': e1, 'ELEMENT_MINOR': e2}
