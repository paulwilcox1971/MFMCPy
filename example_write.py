...
seq[m.H5_PROBE_PLACEMENT_INDEX] = np.ones((1, seq[m.H5_TRANSMIT_LAW].shape[0]))
seq[m.H5_PROBE_LIST] =         np.array([probe_obj.ref])
...
# frame = {}
# frame[m.H5_MFMC_DATA] = np.random.randn(1, seq[m.H5_TRANSMIT_LAW].shape[0], 100)

# for i in range(7):
#     m.fn_add_frame(MFMC, seq_name.name, frame)

#Close file

m.fn_add_frame(MFMC, seq_name, [])
