...
SPEC_TYPE_PREFIX = {
    MFMC_utils.SEQUENCE_TYPE: '{sequence}' + MFMC_utils.H5_PATH_SEPARATOR, 
    MFMC_utils.PROBE_TYPE: '{probe}' + MFMC_utils.H5_PATH_SEPARATOR, 
    MFMC_utils.LAW_TYPE: '{law}' + MFMC_utils.H5_PATH_SEPARATOR}

SPEC_TYPE_COUNTER = {
    MFMC_utils.SEQUENCE_TYPE: '<m>', 
    MFMC_utils.PROBE_TYPE: '<p>', 
   MFMC_utils.LAW_TYPE: '<k>'}
...
def fn_check_sequence(MFMC, SPEC, sequence_name):
    ...
    law_spec = fn_get_relevant_part_of_spec(SPEC, MFMC_utils.LAW_TYPE)
    ...
    for law in law_list_from_sequence:
        ...
        for (p, v, m, i) in zip(probes, vals, max_vals, range(len(probes))):
            if v < 1 or v > m:
                err_list.append('ELEMENT[' + str(i + 1) + '] in law ' + MFMC_utils.fn_name_from_path(MFMC[law].name) + ': refers to elements outside range of probe ' + MFMC_utils.fn_name_from_path(p))
...
def fn_check_mfmc_group_against_specification(MFMC, SPEC, mfmc_group, spec, check_log = [], size_table = {}, err_list = []):
    ...
    for name in list(spec.index):
        base_string = mfmc_group.name + MFMC_utils.H5_PATH_SEPARATOR + name + ': '
